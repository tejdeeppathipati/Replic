...
class Candidate(BaseModel):
    ...
    platform: Literal["x", "reddit", "linkedin"] = Field(..., description="Social platform")
    source_ref: str = Field(..., description="Tweet ID or Reddit post ID")
    proposed_text: constr(max_length=3000) = Field(..., description="Generated reply text")
    persona: Literal["normal", "unhinged", "smart", "technical"] = Field(
        ..., description="Reply persona/tone"
    )
    ...
    owner_whatsapp: str | None = Field(
        None, description="Owner's WhatsApp number (format: whatsapp:+1...)"
    )
...
class Decision(BaseModel):
    ...
    decision: Literal["approved", "edited", "rejected", "expired"] = Field(
